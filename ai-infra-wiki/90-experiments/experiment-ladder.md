...
2. Transformer 组件 smoke：验证 mask、RMSNorm、MHA/GQA shape 和 KV append，见 [[90-experiments/transformer-components-smoke]]。
3. 单机 DDP：观察 gradient all-reduce，见 [[90-experiments/torchrun-ddp-fsdp-minimal]]。
4. 单机 FSDP：比较显存和 step time，见 [[90-experiments/torchrun-ddp-fsdp-minimal]]。
5. Activation checkpoint：比较显存下降和重算成本。
6. 多机 NCCL：测 all-reduce 带宽和 timeout 排查。
7. vLLM/SGLang 单模型 serving：测延迟、吞吐、KV cache，见 [[90-experiments/vllm-sglang-benchmark-harness]]。
8. 长上下文 serving：观察 KV cache 容量和 admission。
9. 量化 serving：比较显存、吞吐、质量。
10. 多租户压测：队列、公平性、拒绝率。
11. 端到端 incident drill：OOM、NCCL hang、latency regression。
